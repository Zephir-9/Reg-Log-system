...
void main_menu();
bool isPrime(InfInt prime);
InfInt calculateE(InfInt t);
InfInt greatestCommonDivisor(InfInt e, InfInt t);
InfInt calculateD(InfInt e, InfInt t);
long long encrypt(long long i, InfInt e, InfInt n);
long long decrypt(long long i, InfInt d, InfInt n);
std::string loadTxtFromFile(std::string);
void Start_encryption(Key public_key, InfInt e, InfInt key_1);
void Start_decryption(/*long d, long key_1*/);
void writeRSAmsg(long long *msg_E, Key public_key, std::string msg);
InfInt getD(std::string);
InfInt getKey(std::string);

void RSA_menu()
{/*setlocale(LC_ALL, "RUS");
	srand(time(nullptr));*/

	InfInt var_one, var_two, key_1, e_func, e, d;
	int count = 0;
	//----------------------
		do
		{
...
void Start_decryption(/*long d, long key_1*/)
{
	...
	InfInt d = getD(msg_path);
	InfInt key_1 = getKey(msg_path);
...
InfInt greatestCommonDivisor(InfInt e_tmp, InfInt t)
{
	while (e_tmp > 0)
	{
		InfInt myTemp;
...
InfInt calculateD(InfInt e_tmp, InfInt e_func)
{
...
InfInt getKey(std::string filename)
{
...
InfInt getD(std::string filename)
{
...
Key::Key(InfInt key_1, InfInt key_2)
{
	key1 = key_1;
	key2 = key_2;
}
